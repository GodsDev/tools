...
//        $this->assertSame(true, Tools::begins($response, '<!doctype html>'));
    ...
    public function testGoogleAuthenticatorCode()
    {
        ...
        $value = (string) $this->tools->GoogleAuthenticatorCode('abc');

        $this->assertSame(
            1,
            preg_match('~^\d+$~', $value),
            sprintf('Expected only digits, got: "%s"', $value)
        );
    }
    ...
    public function testRelativeTime()
    {
        ...
        $enPast = Tools::relativeTime(time() - 1);
        $this->assertSame(1, preg_match('/(1 second ago|2 seconds ago)/', $enPast), $enPast);

        $csPast = Tools::relativeTime(time() - 1, 'cs');
        $this->assertSame(1, preg_match('/(1 vteřina zpátky|2 vteřiny zpátky)/', $csPast), $csPast);

        // to work with PHP7.3
        $enFuture = Tools::relativeTime(date('Y-m-d H:i:s', time() + 1));
        $this->assertSame(1, preg_match('/(in 1 second|in a moment)/', $enFuture), $enFuture);

        $csFuture = Tools::relativeTime(time() + 1, 'cs');
        $this->assertSame(1, preg_match('/(za 1 vteřina|za okamžik)/', $csFuture), $csFuture);
    }
    ...
    public function testResolve()
    {
        ...
        $_SESSION['messages'] = [];
        /**
         * @phpstan-ignore-next-line Strict comparison using === between 5 and 5 will always evaluate to true.
         */
    ...
    public function testAllFromSToU()
    {
        ...
        $this->assertSame(
            ...
            Tools::urlChange(
                [
                    'a' => 1,
                    'color' => null // ('black' == 'white' ? 'b&w' : null)
                ]
            )
        );
