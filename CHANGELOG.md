...
- add automatic tests on GitHub
- add support to `PHP/8.0-8.5` (so now it is `>=7.2 <8.6`)

### Changed

- PHPUnit_Framework_TestCase to PHPUnit\Framework\TestCase
- PHPStan level to 6 (as now level max is showing a lot of errors)

### Deprecated

- remove php-composer-dependencies.yml in next minor release
...
- Tools::preg_max() should return string but returned int|string. Fix by properly adding regular expression for 0
- Tools::redir()
...
- badges: packagist, Lint codebase passing, PHP Composer + PHPUnit passing
...
- umožněn update na PHPUnit 5.\* \* test related classes moved to autoload-dev section in order not to load them when used as library
...
chore: identical to [0.3.4]

## [0.3.4] - 2019-01-25

fix: rolling back the $CURL_OPTIONS for ::curlCall()
...
- addMessage $type can now be also true (for 'success') or false (for 'warning')
- arrayListed() - +self::ARRL_PATTERN and @example in Docs
- fix: escapeIn() bugfix - null as NULL; numbers not in quotes; true and false get transformed to 1 and 0
- add method function add()
- add method arraySearchAssoc()
- add method mb_lcfirst()
- add methods xorCipher(), xorDecipher()
...
- add method anyset
- add method GoogleAuthenticatorCode
- add method str_putcsv
- add methods str_before, str_after
- add method mb_ucfirst
- add methods array_search_i, in_array_i
- add methods whitelist, blacklist
- add method columnName
- add method httpResponse
...
* add method Tools::anyset()

### Changed

- ifnull() - only 1 parameter needed
- begins(), ends() - mb_strlen() extended with $encoding
- curlCall() - call curl_close() at the end
- new functions: str_before(), str_after(), whitelist(), blacklist(), HTTPResponse()
- docs at: anyset()
- HTTPResponse() renamed to httpResponse()
- redir() - adding parameter $HTTPCode (optional, default = 303)
- ifempty($a, $b) - $b not needed to be mentioned
- begins(), ends(), htmlRadio() - converting 2nd argument to array via the ternary operator
- htmlSelectAppend($array, $default) - requiring $array to be array
- shortify() - adding argument $encoding (since it uses mb_XXXX functions)
...
* fix ::ends()
* fix htmlTextInput - <input value=""> - the value attribute always specified. For ""/0/false/null --> value=""
* hotfix - urlChange() - force argument $changes to be an array, set $\_SERVER['QUERY_STRING'] to "" if not set.
* hotfix in arrayRemoveItems()
...
[Unreleased]: https://github.com/GodsDev/tools/compare/v0.4.0...HEAD
[0.4.0]: https://github.com/GodsDev/tools/compare/v0.3.8...0.4.0
